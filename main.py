...
def list_courses(
    course_number: Optional[CourseNumberType] = Query(
        None, description="Filter by Course Number"
    ),
    name: Optional[str] = Query(None, description="Filter by course name"),
    professor_uni: Optional[UNIType] = Query(
        None, description="Filter by professor UNI"
    ),
    credits: Optional[int] = Query(None, description="Filter by course credits"),
    min_strength: Optional[int] = Query(
...
def root():
    return {
        "message": "Welcome to the Columiba University API. See /docs for OpenAPI UI."
    }
